...
export default function ATCPage() {
  ...
  const { searchTerm, setSearchTerm, searchResults } = useAircraftSearch(aircrafts, airports);
  const time = useUtcTime();
  const { formattedTime, isRunning, start, stop, reset } = useTimer();
  const [showTimerPopup, setShowTimerPopup] = useState(false);
  const [isMapLoaded, setIsMapLoaded] = useState(false);

  const drawFlightPlanOnMapRef = useRef<((ac: PositionUpdate, zoom?: boolean) => void) | null>(null);
...
<header className="absolute top-0 left-0 right-0 z-[10010] flex items-start justify-between px-6 pt-5 h-20">
  
  {/* Left Section */}
  <div className="flex items-center gap-6">
    <div className="hidden md:flex flex-col items-center justify-center pt-1"> 
      <Image
        src="/favicon.ico"
        alt="ATC Radar Logo"
        width={32}
    ...
    {isMapLoaded && (
      ...
      <div className="pointer-events-auto w-80 lg:w-96 h-11">
        <SearchBar
  ...
  <div className="absolute left-1/2 -translate-x-1/2 pointer-events-auto pt-0">
    <button 
      onClick={() => setShowTimerPopup(!showTimerPopup)}
      className="flex flex-col items-center justify-center px-4 py-1.5 min-h-[44px] rounded-full border border-white/10 bg-black/40 backdrop-blur-md transition-all hover:bg-black/60 hover:border-cyan-500/50 group"
    >
      <span className="font-mono text-xl font-medium tracking-tight leading-none text-cyan-400 group-hover:text-cyan-300">
        {time} <span className="text-[10px] text-slate-500">UTC</span>
      </span>
      {isRunning && (
        <span className="animate-pulse font-mono text-[9px] uppercase tracking-[0.2em] text-emerald-400 mt-1 leading-none">
          Timer Active
        </span>
      )}
    </button>
  ...
  <div className="pointer-events-auto flex items-center justify-center gap-4 rounded-full border border-white/5 bg-black/20 px-3 py-1.5 min-h-[44px] backdrop-blur-sm">
    ...
    <div className="flex items-center justify-center">
      ...
      {showTimerPopup && (
        <div className="absolute top-20 left-1/2 z-[10011] -translate-x-1/2 w-64 animate-in fade-in zoom-in-95 duration-200">
          <div className="rounded-2xl border border-white/10 bg-slate-950/80 p-5 shadow-2xl backdrop-blur-2xl">
            <div className="mb-4 text-center">
              <span className="text-[10px] uppercase tracking-widest text-slate-500">Chrono</span>
              <div className="font-mono text-3xl font-light text-white">{formattedTime}</div>
            </div>
            <div className="flex gap-2">
              <button 
                onClick={isRunning ? stop : start} 
                className={`flex-1 rounded-xl py-2.5 text-[10px] font-bold uppercase tracking-wider transition-all ${
                  isRunning 
              ...
              <button 
                onClick={reset} 
                className="flex-1 rounded-xl border border-blue-500/20 bg-blue-500/10 py-2.5 text-[10px] font-bold uppercase tracking-wider text-blue-400 transition-all hover:bg-blue-500/20"
              >
                Reset
              </button>
            </div>
      ...
      <main className="absolute inset-0 z-0">
        {isLoading && aircrafts.length === 0 ? (
          ...
          <DynamicMapComponent
            ...
            setDrawFlightPlanOnMap={(func) => { drawFlightPlanOnMapRef.current = func; }}
